...
def send_file_from_path():
    ...
    if token is not None:
        try:
            ...
            logging.error(
                " get file API (/api/file) got wrong arguments, thrown an error :: %s" % e)
        ...
        try:
            ...
            logging.error(
                "File sending has a exception. When sending file, we got :: %s" % e)
            return Response("File you are trying to access is not available to us. Please ask admin to check the server", status=404)
    else:
...
def start_compression():
    ...
    if token is None:
        token = token_validator(request.headers['token'])
    if token is not None:
        # list of file identifiers to compress
        gid_list = request.get_json()['gid']
        gid_list.sort()
        ...
        if os.path.exists(get_zip_path(ugid_of_compressed_file)):
            response_dict = {'process_id': ugid_of_compressed_file,
                             'progress': get_compression_progress(ugid_of_compressed_file)}
            return jsonify(response_dict)

        script_path = os.path.abspath(os.path.dirname(__file__))
        subprocess.call(
            "python3 {}/compression_handler.py {} &".format(script_path, subprocess_params), shell=True)
